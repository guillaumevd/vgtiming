...
class RaceBackupService {
  ...
  async initializeBackupDirectory() {
    ...
    try {
      // Récupérer le répertoire de sortie depuis les paramètres de l'application
      const { getBackendInstance } = require('../index');
      const backend = getBackendInstance();
      
      if (backend?.services?.settings) {
        try {
          const outputDir = await backend.services.settings.getSetting('outputDir');
          if (outputDir && outputDir !== "C:/Users/username/Documents/") {
            this.backupDirectory = outputDir.replace(/\/$/, ''); // Supprimer le slash final si présent
            logger.debug(`Répertoire de sauvegarde récupéré depuis les paramètres: ${this.backupDirectory}`, {
              service: 'race-backup'
            });
          }
        } catch (settingsError) {
          logger.debug('Impossible de récupérer le répertoire depuis les paramètres, utilisation du répertoire par défaut', {
            service: 'race-backup',
      ...
      if (!this.backupDirectory) {
        this.backupDirectory = path.join(require('os').homedir(), 'Documents', 'VG-Timing');
      }
  ...
  async getRaceDataFromDatabase(raceId) {
    try {
      ...
      const db = backend.database.getDatabase();
